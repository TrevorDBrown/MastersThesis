...
\par With changes in the size and scale of the computer comes changes in the means in which people interface with the computer. Human-Computer Interaction (HCI) - a field dedicated to the research, development, and innovation of how people use computing devices - has been a great influence in how the form factor and presentation of the computer has changed over time.\cite{Web-IDFHCI} Early HCI implementations include computer wiring patterns (e.g. ENIAC (TODO: does this count as an abbreviation?)), punch cards (e.g. UNIVAC, IBM 1401), and bit manipulation by means of physical switches (e.g. Altair 8800, IMSAI 8080). In time, the keyboard was introduced, mimicking the familiar typewriters of the day.
...
\par Although, one application of XR seems to elude the public - Heads Up Display (HUD) in automobiles. HUD technology has existed since the mid 20th century, but has been limited in application, namely for use in aircraft.

\par HUD solutions exist for automobiles today, which are either embedded in the dashboard of a vehicle by the manufacturer (either directly in front of or to the side of the driver), or are added by the driver through a third-party device which sits atop the instrument cluster. Each existing solution shares the same problem - limited dynamic information in a static format. Why must this be so? Must HUDs be restricted in presentation and configuration? With the emergence of new AR technology in the form of wearable glasses, a new solution can finally be explored to provide a more dynamic, and more fluid HUD experience to drivers.

\section{Problem Statement}
\par This thesis will explore an application of AR for automobiles in the form of a dynamic HUD. By the end of this thesis, an AR-based, software system spanning multiple devices will be designed and implemented that satisfies the application's needs.
...
- No solution will be explored or developed to detect obstacles outside of embedded systems integrated in the test vehicle (e.g. computer vision (CV) for object recognition).
- No solution will be explored or developed to block other applications from running on the compute device (e.g. operating system (OS) functions).
